...
// São transmitidos os 4 bytes que representam o numero
...
WINSOCKET_STATUS tcp_winsocket::write_int(const int32_t num) const
{
  ...
  {
    cerr << "winsocket write_int error\n";
    return SOCKET_ERROR;
  }
  return iResult;
}
...
WINSOCKET_STATUS tcp_winsocket::read_int(int32_t &num, long milisec) const
{
  ...
  if ( iResult != sizeof(num))
  {
    cerr << "winsocket read_int error\n";
    return SOCKET_ERROR;
  }
  return iResult;
}

// Rotina para ler uma string em um socket
// Para toda mensagem enviada, inicialmente é
// transmitido o número "n" de bytes da mensagem
...
WINSOCKET_STATUS tcp_winsocket::write_string(const string &msg) const
{
  ...
  if (iResult == SOCKET_ERROR)
  {
    cerr << "winsocket write_string (int) error\n";
    return SOCKET_ERROR;
  }
  iResult = write(msg.c_str(),len);
  if ( iResult != len )
  {
    cerr << "winsocket write_string (chars) error\n";
    return SOCKET_ERROR;
  }
  return iResult;
}
...
// Para toda mensagem recebida, inicialmente é
// transmitido o número "n" de bytes da mensagem
// e em seguida os "n" bytes da mensagem.
// Retorna o numero de bytes que foram lidos ou SOCKET_ERROR em caso de erro
WINSOCKET_STATUS tcp_winsocket::read_string(string &msg, long milisec) const
{
  ...
  if (iResult == SOCKET_ERROR)
  {
    cerr << "winsocket read_string (int) error\n";
    msg = "";
    return SOCKET_ERROR;
  }
  if (len > TAM_MAX_MSG_STRING)
  {
    cerr << "winsocket read_string (int too large) error\n";
    msg = "";
    return SOCKET_ERROR;
  ...
  if ( iResult != len )
  {
    cerr << "winsocket read_string (chars) error\n";
    msg = "";
    return SOCKET_ERROR;
  }
